...
from eloo.server.config import config as eloo_config

LOG_LEVEL = eloo_config.log_level
...
eloo_logger.setLevel(current_log_level)

if DEBUG:
    eloo_logger.addFilter(StackInfoFilter())

if current_log_level == logging.DEBUG:
    LOG_TO_FILE = True
    eloo_logger.debug('DEBUG mode enabled.')

eloo_logger.addHandler(get_console_handler(current_log_level))
eloo_logger.addFilter(SensitiveDataFilter(eloo_logger.name))
eloo_logger.propagate = False
eloo_logger.debug('Logging initialized')
...
class LlmFileHandler(logging.FileHandler):
        ...
        if not DEBUG:
            ...
            for file in os.listdir(self.log_directory):
                ...
                try:
                    ...
                    eloo_logger.error('Failed to delete %s. Reason: %s', file_path, e)
    ...
    def emit(self, record):
        ...
        super().emit(record)
        self.stream.close()
        eloo_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1
